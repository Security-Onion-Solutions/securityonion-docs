...
Query Bar
---------
The query bar defaults to ``Group By Name, Module`` which groups the alerts by ``rule.name`` and ``event.module``. The colored bubbles underneath the query bar represent the individual components of the query and the fields to group by. You can click a bubble to remove it.
