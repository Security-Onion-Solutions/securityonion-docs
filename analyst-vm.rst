...
Starting in Security Onion 2.3, the ``so-analyst-install`` script will install a full GNOME desktop environment including Chromium web browser, :ref:`wireshark`, :ref:`networkminer`, and other analyst tools. ``so-analyst-install`` is totally indepenedent of the standard setup process, so you can run it before or after setup or not run setup at all if all you really want is the Analyst VM itself.
