...
Full-time analysts may want to create a dedicated Analyst VM. This is a full GNOME desktop environment including Chromium web browser, :ref:`networkminer`, :ref:`wireshark`, and other analyst tools. This allows you to investigate pcaps and other potentially malicious artifacts without impacting your Security Onion deployment or your workstation.
...
- If you're doing a network installation on CentOS 7 (NOT using our Security Onion ISO image), then in our normal Setup wizard you can choose ``OTHER`` and then choose ``ANALYST``.

- You can manually run ``sudo so-analyst-install``. This script is totally independent of the standard setup process, so you can run it before or after setup or not run setup at all if all you really want is the Analyst VM itself.
