...
By default, the active query list will filter out internal, non-cancellable, and child queries. Therefore, in most
...
Cancelling queries can take several seconds. Consequently, refreshing the active queries may still show that query in the list for 
a short time. 

.. note::

    Not all queries are cancellable. If a query cannot be cancelled then no cancel icon will appear next to that query listing.

.. warning::

    Cancelling internal, system queries can disrupt the Elasticsearch internal processes. Avoid cancelling queries that are not
