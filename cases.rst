...
If you enable this option, then the Cases interface will show more advanced features similar to :ref:`dashboards` and :ref:`hunt`. These advanced features are only enabled temporarily so if you navigate away from the Cases page and then return to the Cases page, it will be back to its default view.
