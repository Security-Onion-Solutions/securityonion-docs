...
To deploy an Elastic agent to an endpoint, go to the :ref:`soc` :ref:`downloads` page and download the proper Elastic agent for the operating system of that endpoint. Within the Elastic Fleet interface, there is a `Add Agent` button - it is not recommended to use this particular method to install the Elastic Agent, as it requires much more manual configuration.
