...
#. Follow the prompts to complete the installation and reboot. Please note that when creating your OS password, there is currently an issue with special characters like ``$`` and ``!``, so avoid those characters for now.
...
#. If you're using CentOS 7 Minimal, you may need to install ``git``:
