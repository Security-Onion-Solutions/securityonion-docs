...
If you have Internet access and want to have ``so-yara-update`` pull YARA rules from a remote Github repo, modify ``/opt/so/saltstack/local/salt/strelka/rules/repos.txt`` to include the repo URL (one per line).
