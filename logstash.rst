...
.. warning::
    ...
    Also keep in mind, when forwarding logs from the manager, Suricata's ``dataset`` value will still be set to ``common``, as it has not yet been process by the Ingest Node configuration.
